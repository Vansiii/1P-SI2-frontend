...
export class IncidentsListComponent implements OnInit, OnDestroy {
  ...
  constructor() {
    ...
    // IMPORTANTE: Preservar campos locales (_isTimedOut, suggested_technician)
    // que no vienen del backend pero son necesarios para la UI
    this.incidentsService.incidents$.subscribe(incidents => {
      const existingIncidents = this.incidents();
      
      // Convertir los incidentes del servicio al formato local
      // y preservar campos locales que no vienen del backend
      const localIncidents: Incident[] = incidents.map(inc => {
        // Buscar el incidente existente para preservar campos locales
        const existing = existingIncidents.find(e => e.id === inc.id);
        ...
        return {
          ...
          technician: null,
          workshop: null,
          // ✅ Preservar campos locales del incidente existente
          suggested_technician: existing?.suggested_technician || null,
          _isTimedOut: existing?._isTimedOut || false
        };
      });
      
      this.incidents.set(localIncidents);
      console.log('✅ Incidents updated from service (local state preserved):', incidents.length);
    });
